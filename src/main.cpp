...
public:
    ...
    Solution(const Problem &prob) { process_list_.resize(prob.machine_size() + 1); }

    std::vector<State> &process_list(const std::size_t machine_id) { return process_list_[machine_id]; }
    const std::vector<State> &process_list(const std::size_t machine_id) const { return process_list_[machine_id]; }

    void validate(const Problem &prob)
    {
...
public:
    Solution solve(const Problem &prob)
    {
        ...
        {
            for (std::size_t order = 0; order < prob.job_length(job_id); order++)
            {
                const auto &op = prob.operation(job_id, order);
                sol.process_list(op.machine_id).emplace_back(op.operation_id, 0);
            }
        }
        return sol;
    }

    void setup_start_time(Solution &sol, const Problem &prob)
    {
        ...
        {
            for (std::size_t order = 0; order + 1 < sol.process_list(machine_id).size(); order++)
            {
                const auto cur_id = sol.process_list(machine_id)[order].operation_id;
                const auto next_id = sol.process_list(machine_id)[order + 1].operation_id;
                graph[cur_id].push_back(next_id);
                counter[next_id]++;
            }
        }
        ...
        for (std::size_t machine_id = 1; machine_id <= prob.machine_size(); machine_id++)
        {
            const auto &list = sol.process_list(machine_id);
            for (std::size_t order = 0; order < list.size(); order++)
            {
                const auto operation_id = sol.process_list(machine_id)[order].operation_id;
        ...
        for (std::size_t machine_id = 1; machine_id <= prob.machine_size(); machine_id++)
        {
            for (auto &elem : sol.process_list(machine_id))
            {
                elem.start_time = 0;
            }
        }
        ...
        while (!que.empty())
        {
            ...
            const auto cur_time = sol.process_list(machine_id)[order_in_machine].start_time;
            // 配る DP
            if (order_in_machine + 1 < sol.process_list(machine_id).size())
            {
                const auto next_start = cur_time + prob.operation(operation_id).process_time;
                sol.process_list(machine_id)[order_in_machine + 1].start_time =
                    std::max(sol.process_list(machine_id)[order_in_machine + 1].start_time, next_start);
            }
            ...
            if (job_order + 1 < prob.job_length(job_id))
            {
                ...
                sol.process_list(next_op.machine_id)[next_order_in_machine].start_time =
                    std::max(sol.process_list(next_op.machine_id)[next_order_in_machine].start_time, next_start);
            }
...
int main(int argc, char *argv[])
{
    ...
    solver.setup_start_time(sol, problem);
